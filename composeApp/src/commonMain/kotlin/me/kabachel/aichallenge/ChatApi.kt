...
class ChatGptApi : ChatApi {
    ...
    override suspend fun sendChatRequest(apiKey: String, request: ChatRequest): ChatResponse {
        ...
        val systemPrompt = """
            ...
            Если текущее собеседование ещё не началось — начни с вопроса, на какую должность пользователь хочет пройти собеседование (предложи варианты: Frontend, Backend, Mobile, Data Science, DevOps).
            Если пользователь уже выбрал направление и подтвердил готовность (например, ответил «да», «готов», «поехали»), не начинай заново, а продолжай текущее собеседование.
            Задавай 3–5 вопросов разной сложности по выбранной теме (от простого к сложному).
            После того как пользователь ответил на вопросы, оцени уровень кандидата (Junior, Middle, Senior), предложи примерную зарплату и дай совет.
            Заверши фразой «Интервью завершено» и выдай результат в JSON:
            {
